...
def create_test_is():
    ...
    # Link Environment to InstallableSet and create intermediate objects:
    #  * ComponentInstance
    #  * ComponentInstanceConfiguration
    #  * Installation
    # Usage : Test scm/delivery/<project>/<iset_id>/

    installation_1 = Installation(asked_in_ticket='ticker lié', install_date=timezone.now(), installed_set=is1)
    installation_1.save()
    ci_1 = ComponentInstance(description=ImplementationDescription.objects.all()[0])
    ci_1.save()
    ci_1.environments.add(Environment.objects.get(id=1))
    ci_1.save()
    cic_1 = ComponentInstanceConfiguration(created_on=timezone.now(), component_instance=ci_1, result_of=InstallableItem.objects.get(id=1), part_of_installation=installation_1)
    cic_1.save()
