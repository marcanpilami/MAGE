...
urlpatterns = [
    ...
    # Progression
    re_path(r'project/(?P<project_id>\d+)/envt/(?P<envt_name>.+)/hist/(?P<limit>\d+)/', views.all_installs, name='envtinstallhistprm'),
    re_path(r'project/(?P<project_id>\d+)/envt/(?P<envt_name>.+)/hist/', views.all_installs, {'limit': 90}, name='envtinstallhist'),

    # Delivery referential
    re_path(r'project/(?P<project_id>\d+)/delivery/$', views.delivery_list, name='deliveries'),
    re_path(r'project/(?P<project_id>\d+)/delivery/(?P<iset_id>\d+)$', views.delivery, name='delivery_detail'),

    # Delivery hand-off forms
    re_path(r'project/(?P<project_id>\d+)/delivery/edit/$', views.delivery_edit, name='delivery_create'),
    re_path(r'project/(?P<project_id>\d+)/delivery/(?P<iset_id>\d*)/edit/$', views.delivery_edit, name='delivery_edit'),
    re_path(r'project/(?P<project_id>\d+)/delivery/(?P<iset_id>\d*)/editdep/$', views.delivery_edit_dep, name='delivery_edit_dep'),
    ...
    re_path(r'project/(?P<project_id>\d+)/version/summary/$', views.lc_versions_per_environment, name='lc_installs_envts'),
    re_path(r'project/(?P<project_id>\d+)/version/lc/$', views.lc_list, name='lc_list'),
    re_path(r'project/(?P<project_id>\d+)/version/lc/(?P<lc_id>\d*)$', views.lc_versions, name='lc_versions'),

    # Tags
    re_path(r'project/(?P<project_id>\d+)/tag/$', views.tag_list, name='tag_list'),
    re_path(r'project/(?P<project_id>\d+)/tag/(?P<tag_id>\d*)$', views.tag_detail, name='tag_detail'),
    re_path(r'project/(?P<project_id>\d+)/tag/create/(?P<envt_name>.*)/(?P<tag_name>.*)$', views.tag_create, name='tag_create'),
    re_path(r'project/(?P<project_id>\d+)/tag/(?P<tag_id>.*)/remove$', views.tag_remove, name='tag_remove'),

    # IS life cycle
    re_path(r'project/(?P<project_id>\d+)/is/(?P<iset_id>\d*)/validate$', views.iset_validate, name='iset_validate'),
    re_path(r'project/(?P<project_id>\d+)/is/(?P<iset_id>\d*)/invalidate$', views.iset_invalidate, name='iset_invalidate'),
    re_path(r'project/(?P<project_id>\d+)/is/(?P<is_id>\d*)/archive$', views.iset_archive, name='iset_archive'),
    re_path(r'project/(?P<project_id>\d+)/is/(?P<is_id>\d*)/unarchive$', views.iset_unarchive, name='iset_unarchive'),
